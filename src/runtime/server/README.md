...
`POST /api/runtime/plugins/publish` 接收 CI 提交的锁定 SHA、清单、artifact 和 SHA-256，并复用安装事务的校验、健康检查、原子激活和回滚。发布凭证由 `plugin_publish_credentials` 按租户和 Git 来源限制，可通过运行时 API 轮换或撤销；不使用全局 CI 发布令牌。
